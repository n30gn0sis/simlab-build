...
2. **Management-path survival.** For anything touching Netplan, routes, firewall, or sshd: is the session-carrying interface identified? Is the existing config saved with a rollback generated? Is `netplan try` (not `netplan apply`) used remotely? Does the UFW enable sequence add the management allow rules and verify them BEFORE enabling? Any "no" = REJECT. (iDRAC recovery proof is not required — dropped 2026-09-24; nothing may touch iDRAC or the PERC.)
