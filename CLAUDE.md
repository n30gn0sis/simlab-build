...
- **You run on the internet-connected staging side.** Bundle building happens on a dedicated **Proxmox VM running Ubuntu 24.04 + Docker CE** (the default, set 2026-09-04) or on **RHEL 8 with rootful podman** (supported alternative, added 2026-09-11) — or any other container runtime that passes the preflight's capability probes (added 2026-09-14) — see `docs/plans/r770-dependency-manifest.md` §0 — via `scripts/r770-offline-fetch.sh`, whole or a section at a time (`--only`, `--skip`, `--list`).
- **Check which box you are on before assuming you can build a bundle.** Run `scripts/r770-staging-preflight.sh` — it answers this and refuses a host that cannot produce a valid bundle. A session may run in a container that is *not* the staging VM: `systemd-detect-virt` must not say `lxc`, a container runtime must answer `info` (docker, podman, nerdctl, or whatever `STAGING_CTR` names) and its `save` must write docker-archive — the preflight probes both, and `/` needs ≥150 GB free. Research, pin review, and document work run anywhere with egress; the fetch itself does not.
...
2. **Management connectivity is sacred.** Before any change to Netplan, firewall, SSH, or routing on the R770: identify the interface carrying the current SSH session, save the existing config, generate a rollback, and use `netplan try` for remote network changes. Losing SSH to the R770 is a critical failure. (No iDRAC recovery-path proof is required — dropped 2026-09-24, see Decisions of record.)
...
Ubuntu 24.04 LTS · Malcolm as the integrated analysis stack (no separate Zeek/Arkime installs; version pin owned by `scripts/r770-offline-fetch.sh` — see `OWNERS.md`) · Suricata disabled initially · GeoIP descoped · OVS deferred · Windows endpoints descoped · curated APT bundle (not a mirror) · **Ubuntu 24.04 Proxmox VM + Docker CE staging by default; RHEL 8 + rootful podman a supported alternative** (default set 2026-09-04, RHEL path added 2026-09-11; **any container runtime accepted on capability, not name, since 2026-09-14** — the preflight probes `info`, egress and the save format, and rootless podman or Docker CE on RHEL warn instead of refuse — run `scripts/r770-staging-preflight.sh` before any fetch) · ext4 256 GB+ transfer media · ad-hoc bundle cadence · **bump moved pins at cut time** (grafana-oss held below 13.x is the standing exception) · **Dell firmware is not a bundle item** (2026-09-25; handled on the R770 directly — `verify` no longer checks `dell/`) · **No iDRAC or PERC work on the R770** (2026-09-24): no firmware, controller-setting, virtual-media or out-of-band actions, and nothing that queries either; the PERC key-custody gate and the iDRAC recovery-path gate are dropped as accepted risks. Changing any of these requires the operator's explicit say-so; record the change in `PRD.md` §6 and `docs/plans/r770-dependency-manifest.md` §0.
