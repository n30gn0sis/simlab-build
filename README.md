...
| `docs/CODEMAPS/` | Token-lean maps for agents: architecture, scripts, config, tests, dependencies. Regenerated by `/ecc:update-codemaps`; not a source of truth |
| `scripts/r770-offline-fetch.sh` | Bundle builder — run on staging, never on the R770; whole, or a section at a time (`--list`, `--only`, `--skip`, `--dry-run`) |
