...
| Staging host | **Default: a dedicated Proxmox VM running Ubuntu 24.04 with Docker CE**, ~300 GB disk (changed 2026-09-04, operator approved; was RHEL 8 with Docker Engine). Rationale for the default: Docker CE is a first-party path on Ubuntu rather than a third-party repo outside Red Hat support that conflicts with the `container-tools` module; it matches the bundle's target OS; the VM can be snapshotted before fetch day; and it removes the SELinux `:Z` bind-mount risk entirely (Ubuntu uses AppArmor). **Supported alternative, added 2026-09-11 (operator approved): RHEL 8 with rootful podman** from `container-tools` — *not* Docker CE, which is precisely the conflict above. The fetch was already portable: every Ubuntu-specific command runs inside a clean `ubuntu:24.04` container, and both bind mounts already carry `:Z`. What the RHEL path adds is `scripts/r770-staging-preflight.sh`, which refuses a host that cannot produce a valid bundle — podman below 3.0 (no `--multi-image-archive`, so the tarballs would be unreadable by the R770's Docker), rootless podman, Docker CE on RHEL, or an LXC container. Either host must be a **VM, not an LXC container**: Docker there needs `nesting=1`/`keyctl=1` and still fights overlayfs. |
