...
2. Management connectivity is sacred. There is no proven out-of-band recovery path (no iDRAC work, decision 2026-09-24), so every network change uses `netplan try` with a saved rollback; the management path is the **802.3ad bond `lacp-trunk` with VLAN 10 on top (`lacp-trunk.10`, 10.10.10.31/24)** — not the integrated NIC, as discovery corrected on 2026-09-03. Neither is ever reconfigured without a validated alternate path.
...
> **PERC encryption (accepted risk, 2026-09-24):** the PERC reports `Encryption mode: Enabled` with a `Security Key Assigned`. Key custody is not being established — no PERC work on the R770 (`PRD.md` §6). Losing the key loses the VD. No phase is gated on it.
...
Measured on the 8 GiB / 6-vCPU staging VM (2026-09-12, heap auto-sized by the installer to OpenSearch 4g / Logstash 2500m): the full `malcolm` profile — 27 services — runs healthy at **~7.4 GiB used**, with OpenSearch 4.6 GiB, Logstash 1.7 GiB, NetBox 0.25 GiB and everything else under 0.1 GiB each. The ~64 GB infrastructure figure above therefore has headroom; the real driver on the R770 is the OpenSearch heap chosen for the index volume, not the rest of the stack.
...
- **Malcolm behind the portal — measured, not assumed (staging rehearsal 2026-09-12, `state/inventory/malcolm-rehearsal-2026-09-12.md`).** Malcolm's stock compose publishes `nginx-proxy` on `0.0.0.0:443` and its installer has no setting for that bind, so compliance with the rule above is a one-line edit to `docker-compose.yml` after every installer run: `sed -i 's|^    - 0.0.0.0:443:443/tcp$|    - 127.0.0.1:8443:443/tcp|' docker-compose.yml`. A `docker-compose.override.yml` does **not** work — `scripts/control.py` passes `-f` explicitly. Behind the portal vhost (`config/nginx/malcolm.lab.conf`, TLS terminated by the portal, `proxy_pass https://127.0.0.1:8443` with Malcolm's self-signed cert unverified) every Malcolm site answered identically to direct access, every redirect was relative, and the stack came up healthy with all 132 outbound egress attempts dropped. Auth mode is `basic` (installer default); Keycloak is present but not in the auth path.
