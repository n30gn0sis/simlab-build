...
|---|---|---|---|
| 1 | Hardware & OS discovery (read-only) | — | No — **DONE, VERIFIED 2026-09-03** |
| 2 | BIOS/firmware/iDRAC assessment; RAID VD verification; **PERC encryption key custody**; **NVMe x2-link investigation** | 1 | Only if changes chosen (each gated) |
| 3 | Storage: new LVs in the existing VG `ubuntu-vg0` | 1,2 | **Lower than planned** — discovery found ≈6.84 TiB of free extents, so this is `lvcreate`/`mkfs`/`mount` plus one online `lvextend` of `lv-var`. **No repartitioning, no RAID change, no reinstall.** Still gated: it writes filesystems |
...
| 5 | Management networking finalized in Netplan (`netplan try`), dnsmasq + chrony serving `.lab` | 4 | **High** — the mgmt path is an 802.3ad bond + tagged VLAN (`lacp-trunk.10`), not a single port. Blocked until iDRAC is proven reachable (§12 risk 2) |
...
## 15. Phase 1 — Read-Only Discovery Commands — **EXECUTED 2026-09-02, results in `state/inventory/`**

> Kept for reference and for re-running after hardware changes. `scripts/r770-precheck.sh` automates all of it; its output for this chassis is `state/inventory/r770-precheck-report-2026-09-02.md`, complemented by the iDRAC export that answered the PERC and NIC-media questions the host could not see.
