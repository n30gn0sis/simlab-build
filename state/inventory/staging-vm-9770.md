...
| Address | **192.168.4.78** — a **DHCP reservation** for MAC `BC:24:11:97:70:01` (operator, 2026-09-25). Before that the lease moved: .72 on 2026-09-24, then .78 after a rollback on 2026-09-25 (the .28 recorded below is older still). The driver default is .78; the session's SSH config offers the staging key to any `192.168.4.*` host |
